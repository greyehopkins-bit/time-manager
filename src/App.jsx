...
import { Plus, Trash2, Calendar as CalendarIcon, CheckCircle2, Timer, BookOpen, Briefcase } from "lucide-react";
...
const Card = ({ className = "", children }) => (<div className={`rounded-2xl shadow-sm border bg-white ${className}`}>{children}</div>);
const CardHeader = ({ className = "", children }) => (<div className={`p-4 border-b ${className}`}>{children}</div>);
const CardContent = ({ className = "", children }) => (<div className={`p-4 ${className}`}>{children}</div>);
const Button = ({ className = "", children, ...props }) => (<button className={`px-3 py-2 rounded-xl border shadow-sm hover:shadow transition ${className}`} {...props}>{children}</button>);
const Input = ({ className = "", ...props }) => (<input className={`w-full px-3 py-2 rounded-xl border focus:outline-none focus:ring ${className}`} {...props} />);
const Textarea = ({ className = "", ...props }) => (<textarea className={`w-full px-3 py-2 rounded-xl border focus:outline-none focus:ring ${className}`} {...props} />);
const Select = ({ className = "", children, ...props }) => (<select className={`w-full px-3 py-2 rounded-xl border bg-white ${className}`} {...props}>{children}</select>);
    ...
    <div className="grid lg:grid-cols-3 gap-6">
      <Card className="lg:col-span-1">
        ...
        <CardContent className="space-y-3">
          ...
          <Select value={status} onChange={e=>setStatus(e.target.value)} className={`w-auto ${statusBadge(status)}`}>
...
function SchoolworkPage() {
  ...
  const [week, setWeek] = useLocalState('ptm_weekplan', { Mon: [], Tue: [], Wed: [], Thu: [], Fri: [], Sat: [], Sun: [] });

  const addTodo = () => {
    const txt = todoText.trim();
    if (!txt) return;
    const item = { id: crypto.randomUUID(), text: txt, done: false, due: todoDue };
    setTodos([item, ...todos]); setTodoText(""); setTodoDue("");
  };
